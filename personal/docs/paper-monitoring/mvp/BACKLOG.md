...
|----|------------|--------|------|----------|--------|
| BL-001 | Knowledge graph visualization as primary dashboard UI | User request during PoC review | 2026-04-16 | Medium | Promoted (M1) |
| BL-002 | Comprehensive textbook seeding (5 books, ~86 chapters) | TASK-029 in TASKS.md, deferred from PoC | 2026-04-16 | High | Promoted (M3) |
| BL-003 | Semantic concept deduplication (embedding-based) | TASK-030 in TASKS.md, deferred from PoC | 2026-04-16 | High | Promoted (M3) |
| BL-004 | Paper decomposition: problem / solution / result structure | User request, SkillClaw example | 2026-04-16 | High | Subsumed by BL-007 |
| BL-005 | External learning gateway (NotebookLM integration) | User request | 2026-04-16 | Medium | Backlogged |
| BL-006 | Split concept relationships: prerequisites vs. alternative approaches | User request | 2026-04-16 | High | Subsumed by BL-007 |
| BL-007 | Knowledge graph redesign: relationship types + extractable information schema | User request, design discussion | 2026-04-16 | High | Promoted (M1) |
| BL-008 | Search: prioritize name and technique/architecture over description fields | User request, current search returns too many irrelevant matches | 2026-04-16 | Medium | Promoted (M4) |
| BL-009 | Concept taxonomy: hierarchical grouping / tagging of concepts | User request, concepts are flat with no navigable structure | 2026-04-16 | High | Promoted (M5) |
| BL-010 | "What changed in my graph" digest + trending concepts + content opportunities | User request, monetization angle | 2026-04-16 | High | Promoted (M6, layers 1-2 only) |
| BL-011 | User interaction layer: read status, inline notes, last accessed, flagged | Design discussion, PKM research | 2026-04-16 | High | Promoted (M4) |
| BL-012 | Manual graph editing: correct/add/remove relationships and node properties | User request, LLM connections will have errors | 2026-04-17 | High | Promoted (M1) |
...
### BL-012: Manual Graph Editing — Correct/Add/Remove Relationships and Node Properties
- **Source**: User request on 2026-04-17 — LLM-generated connections will have errors, and if changes are not allowed, it hurts usability for personal use
- **Context**: The LLM will inevitably produce incorrect relationships (wrong baselines, missing prerequisites, misidentified problems). The user must be the authority over the knowledge graph, not the LLM. Without manual editing, every LLM error persists permanently, eroding trust in the system.

  **Required editing capabilities:**
  | Action | Example |
  |--------|---------|
  | Add edge | "XGBoost ALTERNATIVE_TO LightGBM" (LLM missed this) |
  | Remove edge | "Decision Tree BASELINE_OF Neural Network" (wrong — not a direct baseline) |
  | Change edge type | "Change BUILDS_ON to PREREQUISITE_OF between these two concepts" |
  | Edit node properties | Fix a technique's description, correct innovation_type, update practical_relevance |
  | Add node | Manually create a Problem, Technique, or Concept the LLM didn't extract |
  | Delete node | Remove a noise concept the LLM hallucinated |

  **Design principles:**
  - Edits are immediate — no "save" button, no approval workflow
  - User edits override LLM — if the user says "this edge is wrong," it's wrong
  - Edits persist through re-classification — if a paper is re-run through the pipeline, user-edited edges are preserved (flagged as `user_edited=true`)
  - Edit history: track `edited_by: "user"` vs `edited_by: "llm"` on edges/nodes for transparency

- **PRD Impact**: Major — affects GraphStore (new mutation methods), dashboard (editing UI on graph visualization), pipeline (preserve user edits during re-classification)
- **Effort Estimate**: Medium (GraphStore methods are simple; UI for graph editing is the main work)
- **Decision**: **Promoted to MVP scope** — critical for personal usability. Must be in Milestone 1 (manual prototype depends on it)
...
|-----------|-------------|------|-------|
| BL-007 | Milestone 1 (TASK-001, TASK-002, TASK-005) | 2026-04-17 | Concept-first schema redesign — Pydantic models, GraphStore, hand-crafted prototype. Subsumes BL-004 and BL-006 |
| BL-004 | Subsumed by BL-007 | 2026-04-17 | Paper decomposition (problem/solution/result) is part of BL-007's PaperAnalyzer |
| BL-006 | Subsumed by BL-007 | 2026-04-17 | Split concept relationships covered by BL-007's 7 edge types |
| BL-012 | Milestone 1 (TASK-002, TASK-004) | 2026-04-17 | Manual graph editing — GraphStore mutations + editing UI. Critical for M1 manual prototype and ongoing LLM error correction |
| BL-001 | Milestone 1 (TASK-003) | 2026-04-17 | Graph visualization moved to M1 — needed for prototype validation and editing UI |
| BL-002 | Milestone 3 (TASK-010) | 2026-04-17 | Comprehensive textbook seeding (5 books, ~86 chapters) |
| BL-003 | Milestone 3 (TASK-011, TASK-012) | 2026-04-17 | Semantic concept deduplication (embedding-based) |
| BL-011 | Milestone 4 (TASK-013, TASK-014) | 2026-04-17 | User interaction layer: read status, notes, flagged, last_accessed_at |
| BL-008 | Milestone 4 (TASK-015, TASK-016) | 2026-04-17 | Search ranking with field-priority matching |
| BL-009 | Milestone 5 (TASK-017, TASK-018) | 2026-04-17 | Concept taxonomy — auto-generated hierarchical grouping |
| BL-010 | Milestone 6 (TASK-019, TASK-020, TASK-021) | 2026-04-17 | "What changed in my graph" digest — layers 1-2 only. Layer 3 (content opportunities) deferred |
