...
> Translate the user's needs into UX requirements for **$ARGUMENTS**: user workflow mapping, interaction specification, information architecture, human-in-the-loop patterns, error/empty states. Use `docs/$ARGUMENTS/poc/DISCOVERY-NOTES.md` as the authoritative source — especially any direct user quotes about how they want to use the tool. Produce `docs/$ARGUMENTS/poc/UX-SPEC.md` following the template at `docs/templates/UX-SPEC-TEMPLATE.md`. Applies to every project — even CLI tools, emails, and alerts have UX decisions to make. **If (and only if) the project has a visual UI surface** (web dashboard, rich HTML email, presentation deck, one-pager report) — NOT pure CLI / plain-text email / API / Slack alert — also produce ONE Claude Design Brief for the single highest-risk visual surface at `docs/$ARGUMENTS/poc/designs/<surface-slug>-brief.md` per your agent definition. If the project has no visual surface, skip the brief and report that decision with one-sentence rationale.

If a brief was produced, after Step 3 completes the main session will prompt the user to run Claude Design with that brief and save the prototype to `designs/<surface-slug>-prototype.{url,pdf}`. If no brief was produced, proceed directly to Step 4.
