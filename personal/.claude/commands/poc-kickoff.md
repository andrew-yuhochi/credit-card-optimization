...
> Translate the user's needs into UX requirements for **$ARGUMENTS**: user workflow mapping, interaction specification, information architecture, human-in-the-loop patterns, error/empty states. Use `docs/$ARGUMENTS/poc/DISCOVERY-NOTES.md` as the authoritative source — especially any direct user quotes about how they want to use the tool. Produce `docs/$ARGUMENTS/poc/UX-SPEC.md` following the template at `docs/templates/UX-SPEC-TEMPLATE.md`. Applies to every project — even CLI tools, emails, and alerts have UX decisions to make.
